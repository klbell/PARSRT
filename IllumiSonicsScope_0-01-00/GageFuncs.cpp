...
int saveGageData(LPCTSTR saveDirectory)
{
	...
	for (u32ChannelNumber = 1; u32ChannelNumber <= CsSysInfo.u32ChannelCount; u32ChannelNumber += u32ChannelIndexIncrement)
	{
			...
			nMulRecGroup++, nTimeStampIndex++)
		{
			...
			// We don't even care about channel 4
	...
	// Build output string
	int paddingSize = 2; // How many calibration variables we have
	int outputSize = (nMeasureSize + 2)*nMeasureTotal + paddingSize; // the +2 is for the mirror possition information
	...
	int measNum = 0;
	for (int i = paddingSize; i < outputSize; i += (nMeasureSize + 2))
	{
		// Add mirror data
		...
		for (int j = i + 2, n = 0; j < i + 2 + nMeasureSize; j++,n++)
		{
